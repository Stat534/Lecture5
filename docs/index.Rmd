...
data.frame(dist = as.numeric(dist.mat), diff = as.numeric(dist.sq)) %>% mutate(bin = floor(dist / bin.width) + 1) %>% filter(bin < 16) %>% group_by(bin) %>% summarize(emp.sv = .5 * mean(diff)) %>% ggplot(aes(x=bin, y= emp.sv)) + geom_point() + xlim(0,15) + ylim(0,800)

## CONTINUE HERE, need to clean up image and verify
